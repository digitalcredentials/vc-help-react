export const VCHelp = () => {
  return <>
    ...
    we must be told from a trustworthy source the public key does in fact belong
    to the claimed issuer. We can be told directly by the issuer themselves, or they might post it on their well known well
    site so we can check it as needed, or they might add it to a registry so that it can be looked up along with the 
    keys of other issuers.
      ...
      <p>It is fundamentally necessary that we are told in some trustworthy way that a public key does belong to the issuer. Without that knowledge,
        fake key pairs could be used to sign fake credentials and we would not know.
      </p>
      <p>Here, when verifying a credential, we look up public keys in a registry controlled by the Digital Credentials Consortium.
